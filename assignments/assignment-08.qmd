...
-   Implement a Metropolis-Hastings-Algorithm for obtaining MCMC samples of $\mu \mid Y=y$ using $N(\tilde{y}, 0.6)$ as *fixed independent proposal distribution for all iterations* (note that this is an asymmetric proposal from the MH perspective!). Here, $\tilde{y}$ is the MAP.
